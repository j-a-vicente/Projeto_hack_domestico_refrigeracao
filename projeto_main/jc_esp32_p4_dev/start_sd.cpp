...
// Cria as tabelas dos objetos de monitoramento
void criarTabelasDeMonitoramento(sqlite3* db) {
    ...
    if (sqlite3_prepare_v2(db, sqlBusca, -1, &stmt, NULL) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            std::string nomeTabela = "monitor_" + std::string((const char*)nome) + "_" + std::string((const char*)sigla);
...
// Lista as tabelas do banco de dados e mosta no log do serial
...
// Atualiza a tabela configuracoes com a hora do boot
void registrarBootNoBanco(sqlite3* db) {
    Serial.println("\n--- Registrando Boot do Sistema ---");
    
    // O SQLite pega automaticamente a hora local do ESP32 já sincronizada via NTP
    const char* sql = "UPDATE configuracoes SET valor = datetime('now', 'localtime') WHERE sigla = 'START_BOOT_ESP32P4CWEB';";
    
    char* errMsg = nullptr;
    if (sqlite3_exec(db, sql, 0, 0, &errMsg) != SQLITE_OK) {
        Serial.printf("SQLite: Erro ao atualizar a data de boot: %s\n", errMsg);
        sqlite3_free(errMsg);
    } else {
        // Verifica se alguma linha foi realmente alterada (caso a sigla não exista na tabela)
        if (sqlite3_changes(db) > 0) {
            Serial.println("SQLite: START_BOOT_ESP32P4CWEB atualizado com sucesso!");
        } else {
            Serial.println("SQLite: Aviso - Sigla START_BOOT_ESP32P4CWEB não encontrada na tabela configuracoes.");
        }
    }
    Serial.println("-----------------------------------\n");
}
...
// Abre o banco e dispara a criação das tabelas
bool iniciarBancoDeDados() {
    ...
    if (sqlite3_open("/sdcard/data/database/banco_skyonix.sqlite", &db) == SQLITE_OK) {
